...
스크립트 실행 전 루트 `.env`, `backend/.env`, `frontend/.env`를 직접 만들어 둔 경우 기존 파일은 덮어쓰지 않습니다. 외부 공개 환경에서는 `CORS_ORIGINS`와 `VITE_API_BASE_URL`이 실제 접속 주소를 가리키도록 확인하세요. `CORS_ORIGINS` 예시는 JSON 배열 형식을 사용하지만, 여러 origin을 쉼표로 구분한 문자열도 지원합니다.
...
- `CORS_ORIGINS`: 허용할 프론트엔드 origin 목록. JSON 배열 문자열 예: `["http://localhost:15173"]`; 쉼표 구분 문자열 예: `http://localhost:15173,http://127.0.0.1:15173`도 지원합니다.
